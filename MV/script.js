...
async function handleRegister(userData) {
    try {
        ...
        if (result.success) {
            showNotification('Rejestracja zakończona pomyślnie! Możesz się zalogować.', 'success');
            closeAuthModal();
        ...
        showNotification('Błąd podczas rejestracji. Spróbuj ponownie.', 'error');
    }
}
...
async function handleLogin(credentials) {
    try {
        ...
        if (result.success) {
            showNotification(`Witaj, ${result.user.username}!`, 'success');
            closeAuthModal();
        ...
        showNotification('Błąd podczas logowania. Spróbuj ponownie.', 'error');
    }
}
...
function updateAuthUI(user) {
    ...
    if (accountMenu) {
        const dropdown = accountMenu.querySelector('.dropdown-content');
        if (dropdown) {
            // Remove login/register links
            const loginLink = dropdown.querySelector('a[onclick*="login"]');
            const registerLink = dropdown.querySelector('a[onclick*="register"]');
            ...
            dropdown.insertBefore(logoutLink, dropdown.firstChild);
            
            // Update account button text
            const accountBtn = accountMenu.querySelector('.account-btn');
            if (accountBtn) {
...
// Call checkAuthStatus on DOMContentLoaded
...
function saveChapterAsRead(mangaId, chapterNum) {
...
function isChapterRead(mangaId, chapterNum) {
    const history = getReadingHistory();
    const mangaKey = `manga_${mangaId}`;
    return history[mangaKey] && history[mangaKey].includes(chapterNum);
}

// Функция для удаления истории чтения манги
function deleteMangaHistory(mangaIdToDelete) {
    const history = getReadingHistory();
    const mangaKey = `manga_${mangaIdToDelete}`;
    
    if (history[mangaKey]) {
        delete history[mangaKey];
        localStorage.setItem('mangaReadingHistory', JSON.stringify(history));
        showNotification(`История чтения для ${mangaIdToDelete} удалена`, 'success');
        
        // Если удаляем текущую мангу, перенаправляем на главную страницу
        if (window.location.pathname.includes('reader.html')) {
            const urlParams = new URLSearchParams(window.location.search);
            const currentMangaId = urlParams.get('manga_id');
            
            if (currentMangaId === mangaIdToDelete) {
                showNotification('Перенаправление на главную страницу через 2 секунды...', 'info');
                setTimeout(() => {
...
function clearAllHistory() {
    if (confirm('Вы уверены, что хотите удалить всю историю чтения?')) {
        localStorage.removeItem('mangaReadingHistory');
        showNotification('Вся история чтения удалена', 'success');
    }
}
